exportGraph <- function ( p, file.name='plotit-output', file.type='pdf' ) {
...
for (i in 1:length(dset1[, 1])) {
    pl <- coincidencePlot227(as.matrix(dset1[i, 3:9]));
...
for (i in 1:length(dset2[, 1])) {
    exportGraph(loadPlot227(as.matrix(dset2[i, 3:8])), file.name=paste(dset2[i, 2], dset2[i, 1], 'bands'), file.type=1800);
}
