...
sub parse {
    ...
    if ( $self->_check_signature($file) ) {
        require Spreadsheet::ParseXLSX::Decryptor;
        my $decrypted_file =
          Spreadsheet::ParseXLSX::Decryptor->open( $file, $self->{Password} );
...
sub _parse_workbook {
    ...
    my $themes =
      $self->_parse_themes( ( values %{ $files->{themes} } )[0] );    # XXX
    ...
    if ( $files->{strings} ) {
        my %string_parse_data =
          $self->_parse_shared_strings( $files->{strings}, $themes->{Color} );
        ...
        twig_roots => {
            ...
            's:dimension' => sub {
                ...
                my ( $rmin, $cmin, $rmax, $cmax ) =
                  $self->_dimensions( $dimension->att('ref') );
            ...
            's:headerFooter' => sub {
                ...
                my ( $helem, $felem ) =
                  map { $hf->first_child("s:$_") } qw(oddHeader oddFooter);
            ...
            's:pageSetup' => sub {
                ...
                $sheet->{Scale} =
                  ...
                  : 100;
                $sheet->{Landscape} =
                  ( $setup->att('orientation') || '' ) ne 'landscape';
                $sheet->{PaperSize} =
                ...
                $sheet->{UsePage} =
                  $self->_xml_boolean( $setup->att('useFirstPageNumber') );
            ...
            's:col' => sub {
                ...
                for my $colnum ( $col->att('min') .. $col->att('max') ) {
                    ...
                    $columns_hidden[ $colnum - 1 ] =
                      $self->_xml_boolean( $col->att('hidden') );
                }
            ...
            's:sheetPr/s:tabColor' => sub {
                ...
                $sheet->{TabColor} =
                  $self->_color( $sheet->{_Book}{Color}, $tab_color );
            ...
            's:sheetData/s:row' => sub {
                ...
                $rows_hidden[$row_idx] =
                  $self->_xml_boolean( $row_elt->att('hidden') );
                ...
                for my $cell ( $row_elt->children('s:c') ) {
                    ...
                    if ($loc) {
                        ...
                        if ( $row != $row_idx ) {
                            warn
"mismatched coords: got $loc for cell in row $row_idx";
                        }
                        $col_idx = $col - 1;
                    }
                    else {
    ...
    $sheet->{ColWidth} =
      [ map { defined $_ ? 0 + $_ : 0 + $default_column_width }
          @column_widths ];
            ...
            if ( $chunk->name eq 's:t' ) {
                ...
                for my $node_format ( $chunk->children ) {
                    if ( $node_format->name eq 's:sz' ) {
                        ...
                        $format_text{Color} =
                          $self->_color( $theme_colors, $node_format );
                    }
        ...
        my $xml = $self->_new_twig(
            twig_handlers => {
                's:si' => sub {
                    ...
                    my %text_rich =
                      $self->_get_text_and_rich_font_by_cell( $si,
                        $theme_colors );
...
# http://social.msdn.microsoft.com/Forums/en-US/oxmlsdk/thread/e27aaf16-b900-4654-8210-83c5774a179c
    ...
    if ( !$styles ) {
        ...
        my $format =
          Spreadsheet::ParseExcel::Format->new( %default_format_opts,
            Font => $font, );
    ...
    my @fills = map {
        ...
        [
            ...
            $self->_color(
                $workbook->{Color}, $_->first_child('s:fgColor'), 1
            ),
            $self->_color(
                $workbook->{Color}, $_->first_child('s:bgColor'), 1
            ),
        ]
    ...
    my @borders = map {
        my $border = $_;
        my ( $ddiag, $udiag ) = map { $self->_xml_boolean( $border->att($_) ) }
        ...
        {
            colors => [
                map { $self->_color( $workbook->{Color}, $bordercolors{$_} ) }
                  qw(left right top bottom)
            ],
            styles => [
                map { $border{ $borderstyles{$_} } } qw(left right top bottom)
            ],
            diagonal => [
                (
                      $ddiag  && $udiag  ? 3
    ...
    my %format_str = (
        %default_format_str,
        (
            map { $_->att('numFmtId') => $_->att('formatCode') }
...
sub _extract_files {
    ...
    my ($strings_xml) =
      map { $self->_zip_file_member( $zip, $get_path->( $_->att('Target') ) ) }
      $wb_rels->find_nodes(
        qq<//packagerels:Relationship[\@Type="$type_base/sharedStrings"]>);

    my ($styles_xml) =
      map { $self->_parse_xml( $zip, $get_path->( $_->att('Target') ) ) }
      $wb_rels->find_nodes(
